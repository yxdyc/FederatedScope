...
from scripts.personalization_exp_scripts.pfl_bench.res_analysis_plot.\
    render_paper_res import \
    highlight_tex_res_in_table

api = wandb.Api()

name_project = "daoyuan/pFL-bench"
name_project = "daoyuan/pFL-bench-0.2"
...
filters_each_line_twitter = OrderedDict(
    # {dataset_name: filter}
    [
        ("twitter", {
            "$and": [
                {
                    "config.data.type": "twitter_glove"
                },
            ]
        }),
    ])
...
column_names_generalization_v2 = [
    "best_client_summarized_weighted_avg/test_acc",
    "best_unseen_client_summarized_weighted_avg/test_acc", "participation_gap"
]
column_names_fair = [
...
sorted_method_name_pair_fedprox_hypcluster = [
    ("fedprox", "FedProx"),
...
sorted_method_name_pair_rec = [
    ...
    # the mf model does not have the bn parameter
    ("global-train", "Global-Train"),
    ("isolated-train", "Isolated"),
    ("fedavg", "FedAvg"),
    ("fedavg-ft", "FedAvg-FT"),
...
expected_datasets_name_twitter = ["twitter"]
expected_datasets_name_main_table = [
    "sst2",
    "pubmed",
    "FEMNIST-s02",
]
# expected_seed_set = ["0"]  # for twitter
expected_seed_set = ["1"]  # for fedprox and hypcluster
...
for expname_tag in expected_expname_tag_rec:
    for metric in column_names_generalization_loss + \
                  column_names_efficiency + column_names_fair_loss:
        all_res_structed_rec[expname_tag][metric] = "-"

# for twitter and rec datasets, we use the same methods list
expected_expname_tag_twitter = set()
for method_name in expected_method_names_rec:
    for dataset_name in expected_datasets_name_twitter:
        for seed in expected_seed_set:
            expected_expname_tag_twitter.add(
                f"{method_name}_{dataset_name}_seed{seed}")

all_res_structed_twitter = defaultdict(dict)
for expname_tag in expected_expname_tag_twitter:
    for metric in column_names_generalization_v2 + column_names_efficiency + \
                  column_names_fair:
        all_res_structed_twitter[expname_tag][metric] = "-"
sorted_method_name_to_print_twitter = copy.deepcopy(
    sorted_method_name_to_print_rec)

# meta dict for fedprox & hypcluster
sorted_method_name_to_print_prox_cluster = OrderedDict(
    sorted_method_name_pair_fedprox_hypcluster)
expected_keys_prox_cluster = set(
    list(sorted_method_name_to_print_prox_cluster.keys()))
expected_method_names_prox_cluster = list(
    sorted_method_name_to_print_prox_cluster.values())
expected_expname_tag_prox_cluster = set()

for method_name in expected_method_names_prox_cluster:
    for dataset_name in expected_datasets_name_main_table:
        for seed in expected_seed_set:
            expected_expname_tag_prox_cluster.add(
                f"{method_name}_{dataset_name}_seed{seed}")

all_res_structed_prox_cluster = defaultdict(dict)
for expname_tag in expected_expname_tag_prox_cluster:
    for metric in column_names_generalization_v2 + column_names_efficiency + \
                  column_names_fair:
        all_res_structed_prox_cluster[expname_tag][metric] = "-"
...
order_loss = '+' + 'summary_metrics.best_client_summarized' \
                   '_weighted_avg/val_avg_loss'
...
def print_table_datasets_list(filters_each_line_table,
    ...
    for data_name in filters_each_line_table:
        ...
        for sweep_id in sweep_ids:
            ...
            if best_run.state != "finished":
                print(f"==================Waring: the best_run "
                      f"with id={best_run} has state {best_run.state}. "
            ...
            if "isolated" in run_header or "global" in run_header:
                try:
                    ...
                    print(
                        f"KeyError with key={column_names_generalization[0]},"
                        f" sweep_id={sweep_id}, sweep_name={run_header}, "
                ...
                for column_name in column_names_generalization[0:2]:
                    try:
                        ...
                        print(f"KeyError with key={column_name}, "
                              f"sweep_id={sweep_id}, sweep_name={run_header}, "
                              f"best_run_id={best_run.id}")
                        wrong_sweep = True
                if wrong_sweep:
            ...
            for column_name in column_names_efficiency:
                try:
                    ...
                    if not contain_unit:
                        if float(res) < 0:
                            print(f"sth wrong when {column_name}, "
                                  f"got negative value as {res}")
                        res = bytes_to_unit_size(abs(float(res)))
            ...
            is_ft = best_run_cfg["finetune"]["before_eval"] is True or \
                    ("finetune" in best_run_cfg["trainer"] and best_run_cfg[
                        "trainer"]["finetune"]["before_eval"] is True)
            if is_ft and "ft" not in run_header:
                run_header = run_header + ",ft"
            if not is_ft and "ft" in run_header:
            ...
            if run_header not in res_of_all_sweeps:
                ...
                print(f"processed duplicated sweep with name "
                      f"{run_header}, plz check it with id {sweep_id}. "
                      f"The first appeared sweep "
                      f"has id {sweep_name_2_id[run_header]}")
            ...
            # pre-process the expname,
            # step 1. split by ",", get the method header atomic elements
            ...
            for sub in split_res:
                ...
                if "femnist" in sub or "cifar" in sub or "cora" in sub \
                        or "cola" in sub or "pubmed" in sub or \
                        "citeseer" in sub or "sst2" in sub \
                        or "s02" in sub or "s005" in sub or "s01" in sub \
                        or "alpha5" in sub or "alpha0.5" in sub or \
                        "alpha0.1" in sub \
                        or "movielen" in sub or "twitter" in sub:
                    pass
                else:
                    filter_split_res.append(sub)
            # pre-process the expname,
            # step 2. combining the method header elements with "-"
            ...
            # `    continue
            exp_name_current = \
                f"{sorted_method_name_to_print[method_header]}" \
                f"_{data_name}_seed{cur_seed}"
            for i, metric in enumerate(column_names_generalization):
                all_res_structed[exp_name_current][metric] = \
                res_all_generalization[i]
            for i, metric in enumerate(column_names_efficiency):
                all_res_structed[exp_name_current][metric] = \
                res_all_efficiency[i]
            ...
            yaml_f_name = \
                f"{sorted_method_name_to_print[method_header]}" \
                f"_{data_name}.yaml"
            print(f"will save best_runs to {yaml_f_name}")
        ...
        for missing_header in unseen_keys:
            print(f"the header is missing {missing_header} "
                  f"in dataset {data_name}")
    ...
    colum_order_per_data = ["-", "-", "-"] if is_loss else ["+", "+", "+"]
    # for the loss,
    ...
    dataset_num = len(list(filters_each_line_table.keys()))
    for key in sorted_method_name_to_print:
        ...
        for i in range(dataset_num):
            try:
                res_of_each_line_commu_acc_trade[key].extend(
                    ...
                    ["{:.2f}".format(v * times_ratio) if v != "-" else v for v
                     in
...
def print_res_rec():
    ...
    # for expname_tag in expected_expname_tag_rec:
    #    for metric in column_names_generalization_loss +
    #    column_names_efficiency + col#umn_names_fair_loss:
...
def print_res_twitter():
    print_table_datasets_list(
        filters_each_line_twitter,
        order_acc,
        all_res_structed_twitter,
        column_names_generalization_v2,
        column_names_efficiency,
        column_names_fair,
        expected_keys=expected_keys_rec,
        sorted_method_name_to_print=sorted_method_name_to_print_twitter,
    )
    for expname_tag in expected_expname_tag_twitter:
        for metric in column_names_generalization_v2 + \
                      column_names_efficiency + column_names_fair:
            try:
                if all_res_structed_twitter[expname_tag][metric] == "-":
                    print(f"Missing {expname_tag} for metric {metric}")
            except Exception as e:
                print(f"faced exception {e}")

    with open('best_res_all_metric_twitter.json', 'w') as fp:
        json.dump(all_res_structed_twitter, fp)


def print_res_fedprox_hypcluster():
    # datasets are main table
    print_table_datasets_list(
        filters_each_line_main_table,
        order_acc,
        all_res_structed_prox_cluster,
        column_names_generalization_v2,
        column_names_efficiency,
        column_names_fair,
        expected_keys=expected_keys_prox_cluster,
        sorted_method_name_to_print=sorted_method_name_to_print_prox_cluster)

    for expname_tag in expected_expname_tag_prox_cluster:
        for metric in column_names_generalization_v2 + \
                      column_names_efficiency + column_names_fair:
            if all_res_structed_prox_cluster[expname_tag][metric] == "-":
                print(f"Missing {expname_tag} for metric {metric}")

    with open('best_res_all_metric_fedprox_hypcluster.json', 'w') as fp:
        json.dump(all_res_structed_prox_cluster, fp)


# print_res_non_rec()
# print_res_rec()
# print_res_twitter()
print_res_fedprox_hypcluster()
