...
def print_table_datasets_list(filters_each_line_table):
    ...
    for data_name in filters_each_line_table:
        ...
        for best_run in runs_ids:
            ...
            if best_run.state != "finished":
                print(f"==================Waring: the best_run with"
                      f" id={best_run} has state {best_run.state}. ")
            ...
            if old_run_header != run_header:
                print(f"processed {old_run_header} to new "
                      f"run header {run_header}")
            ...
            if run_header not in res_of_all_sweeps:
                ...
                print(f"processed duplicated sweep with name {run_header}"
                      f", plz check it with id {sweep_id}. "
                      f"The first appeared sweep has "
                      f"id {sweep_name_2_id[run_header]}")
            ...
            for sub in split_res:
                if "femnist" in sub or "cifar" in sub or "cora" in sub \
                        or "cola" in sub or "pubmed" in sub \
                        or "citeseer" in sub or "sst2" in sub \
                        or "s02" in sub or "s005" in sub or "s01" in sub \
                        or "alpha5" in sub or "alpha0.5" in sub \
                        or "alpha0.1" in sub:
                    pass
                else:
            ...
            yaml_f_name = \
                f"best_{sorted_keys[method_header]}_on_{data_name}.yaml"
        ...
        for missing_header in unseen_keys:
            print(f"the header is missing "
                  f"{missing_header} in dataset {data_name}")
    ...
    #     res_to_print = ["{:.2f}".format(v * 100)
    #     if v != "-" else v for v in res_of_all_sweeps[key]]
    #     res_to_print = [key] + res_to_print
    #     print(",".join(res_to_print))
    #


def generate_repeat_scripts(best_cfg_path,
                            seed_sets=None,
                            target_words=None,
                            background=False):
    file_cnt = 0
    ...
    for file_name in onlyfiles:
        ...
        for seed in seed_sets:
            cmd = f"python federatedscope/main.py --cfg " \
                  f"scripts/personalization_exp_scripts/pfl_bench/" \
                  f"yaml_best_runs/{file_name} seed {seed} expname_tag " \
                  f"{exp_name}_seed{seed} " \
                  f"wandb.name_project pfl-bench-best-repeat"
            if background:
                cmd = f"nohup {cmd} &"
            print(cmd)
            file_cnt += 1
            if file_cnt % 4 == 0:
                print(f"Seed={seed}, totally generated "
                      f"{file_cnt} run scripts\n\n")
...
seed_sets = [2, 3]
target_words = ["twitter"]
target_words = ["FedProx", "HypCluster"]
for seed in seed_sets:
    # generate_repeat_scripts("/mnt/daoyuanchen.cdy/FederatedScope/scripts
    # /personalization_exp_scripts/pfl_bench/yaml_best_runs", seed_sets=[seed])
    generate_repeat_scripts(
        "/mnt/daoyuanchen.cdy/FederatedScope/scripts"
        "/personalization_exp_scripts/pfl_bench/"
        "yaml_best_runs",
        seed_sets=[seed],
