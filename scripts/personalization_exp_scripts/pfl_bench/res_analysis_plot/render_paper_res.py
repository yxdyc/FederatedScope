...
def load_best_repeat_res(filter_seed_set=None):
    for expname_tag in expected_expname_tag:
                ...
                for metric in column_names_generalization \
                    ...
                    try:
                        if method in ["Isolated", "Global-Train"]:
                            skip_generalize = "unseen" in metric \
                                              or metric == "participation_gap"
                            skip_global_fairness = \
                                method == "Global-Train" and \
                                "fairness" in metric
                        ...
                        if metric == "participation_gap":
                            all_res_structed[expname_tag][metric] = \
                                all_res_structed[expname_tag][
                                    "best_unseen_client_summarized_"
                                    "weighted_avg_unseen/test_acc"] - \
                                all_res_structed[expname_tag][
                                    "best_client_summarized_"
                                    "weighted_avg/test_acc"]
                        else:
                            all_res_structed[expname_tag][metric] = \
                                run.summary[metric]
                    except KeyError:
        ...
        if finished_run_cnt == 0 and print_missing:
            ...
            all_missing_scripts[seed].append(
                f"python federatedscope/main.py --cfg scripts/"
                f"personalization_exp_scripts/pfl_bench/yaml_best_runs"
                f"/{yaml_name} seed {seed_num} expname_tag {expname_tag_new} "
    ...
    for seed in all_missing_scripts.keys():
        print(f"+================= All MISSING SCRIPTS, seed={seed} "
              f"=====================+, cnt={len(all_missing_scripts[seed])}")
        ...
        if "repeat" in expname_tag:
            ...
            for metric in column_names_generalization + \
                ...
                if all_res_structed[expname_tag][
                    metric] == "-" and "Global" not in expname_tag \
                        and "Isolated" not in expname_tag:
    ...
    for col_i, col in enumerate(res_np[:, 1:].T):
        ...
        for raw_i in range(row_len):
            if ind_all_method[raw_i] == 1:
                res_to_print_matrix[raw_i][col_i + 1] = "\\textbf{" + \
                                                        res_to_print_matrix[
                                                            raw_i][
                                                            col_i + 1] + "}"
            if ind_all_method[raw_i] == 2:
                res_to_print_matrix[raw_i][col_i + 1] = "\\underline{" + \
                                                        res_to_print_matrix[
                                                            raw_i][
                                                            col_i + 1] + "}"
            if raw_i in selected_method_idx and ind_partial_method_tmp[
                    raw_i_to_selected_i[raw_i]] == 1:
                res_to_print_matrix[raw_i][col_i + 1] = "\\color{red}{" + \
                                                        res_to_print_matrix[
                                                            raw_i][
                                                            col_i + 1] + "}"
            if raw_i in selected_method_idx and ind_partial_method_tmp[
                    raw_i_to_selected_i[raw_i]] == 2:
                res_to_print_matrix[raw_i][col_i + 1] = "\\color{blue}{" + \
                                                        res_to_print_matrix[
                                                            raw_i][
                                                            col_i + 1] + "}"
...
with open('best_res_all_metric.json', 'r') as fp:
    ...
    for expname_tag in expected_expname_tag:
        ...
        for metric in column_names_generalization + \
                      column_names_efficiency + column_names_fair:
            all_res_structed[expname_tag][metric] = \
                all_res_structed_load[expname_tag][metric]
