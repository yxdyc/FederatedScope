...
class NLPTrainer(GeneralTorchTrainer):
    ...
    def _hook_on_batch_forward_flop_count(self, ctx):
        if not isinstance(self.ctx.monitor, Monitor):
            logger.warning(
                f"The trainer {type(self)} does contain a valid monitor, this "
                f"may be caused by initializing trainer subclasses without "
                f"passing a valid monitor instance."
        ...
        if self.cfg.eval.count_flops and self.ctx.monitor.flops_per_sample \
            ...
            try:
                ...
                logger.error(
                    ...
                    "NLPTrainer case: "
                    "1) the ctx.model takes only x (for Object) or "
